...
async def generate_gantt(plan: dict, db: AsyncSession = Depends(get_db)):
    try:
        ...
        plan_for_json = {k: v for k, v in plan.items() if k not in ['name', 'centre_name', 'program', 'level', 'duration', 'grade']}
        user_plan = UserPlan(
            name=name,
...
async def update_user_duration(duration_data: dict):
    try:
        ...
        return {"message": "Duration updated successfully", "duration": duration_data["duration"]}
    except Exception as e:
...
async def get_topics(program: str, level: str, duration: str, month: str, week: str, grade: str = None):
    try:
